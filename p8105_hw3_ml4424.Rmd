...
jan_jul_mean_temps
jan_mean_temps <- jan_jul_mean_temps %>% 
  filter(month == "01")
jul_mean_temps <- jan_jul_mean_temps %>% 
  filter(month == "07")
mean(jan_mean_temps$mean_tmax)
mean(jul_mean_temps$mean_tmax)
...
# # for plot, have x-axis be study month; you see seasonal trends but NOT THE BEST VIZ
# all_temp_months <- all_temp %>% 
#   tibble::rowid_to_column("month_num")
# all_temp_months
# 
# # plot
# ggplot() +
#   geom_line(data = all_temp_months,
#             aes(x = month_num, y = mean_tmax, color = "red")) +
#   geom_line(data = all_temp_months,
#             aes(x = month_num, y = mean_tmin, color = "blue"))

## alternative: just plot annual averages
...
annual_temps %>% 
  pivot_longer( tmax:tmin, names_to =
"observation"
, values_to =
"temp"
) %>%

annual_temps %>% ggplot(aes(x=year, y = mean_tmax, color = month)) +
  geom_point(alpha = 0.5) + 
  geom_smooth() 

# annual_temp_plot <- ggplot() +
#   geom_line(data = annual_temps,
#             aes(x = year, y = mean_tmax, color = tmax)) +
#   geom_line(data = annual_temps,
#             aes(x = year, y = mean_tmin, color = tmin))
